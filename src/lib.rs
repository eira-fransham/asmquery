...
//! implicitly in the `add` - but it's great that we get some optimisations implemented for
//! free. This algorithm also implicitly prevents patterns like the following:
//!
//! ```
//! %2 = add %0, %1
//! %3 = sub %1, %2
...
//! for _just_ the carry, but since there is no instruction that has a `carry` output for a
//! register provided as input (only instructions that have a `carry` for other outputs) it will
//! cause no instructions to be found and the compiler will error out. However, sequences like
//! the following _will_ succeed, since `test` can be emitted:
//!
//! ```
//! %2 = add %0, %1
//! %3 = sub %1, %2
...
trait LightbeamBackend {
    ...
    fn add(
        ...
        let ret = regs.next();
        (ret, vec![Directive::new(ret, outputs::Generic::Add, vec![a, b])])
    }
}
